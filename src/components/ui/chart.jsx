...
const sanitizeKey = (k) => String(k).replace(/[^a-zA-Z0-9-_]/g, "");
const isHex = /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/;
const sanitizeColor = (c) => (typeof c === "string" && isHex.test(c.trim()) ? c.trim() : null);

const ChartStyle = ({ id, config }) => {
  if (!config || typeof config !== "object") return null;

  const colorConfig = Object.entries(config).filter(([, cfg]) => cfg && (cfg.theme || cfg.color));
