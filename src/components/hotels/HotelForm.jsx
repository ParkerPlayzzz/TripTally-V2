...
export default function HotelForm({ onSubmit, onCancel, initial }) {
  ...
  const handleLogoUpload = async (e) => {
    ...
    try {
      let logoUrl = "";
      if (db?.integrations?.Core?.UploadFile) {
      ...
      setUploadError("Unable to upload logo. Please try again.");
      console.error("HotelForm logo upload failed", error);
    } finally {
      setUploading(false);
      if (fileRef.current) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="h-9 w-full rounded-xl border border-gray-200 bg-transparent px-3 text-sm text-foreground outline-none focus:border-gray-400 focus:ring-gray-400"
              >
