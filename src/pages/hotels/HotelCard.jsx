    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-lg hover:shadow-gray-100 transition-all duration-300 group">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-gray-50 rounded-xl px-3 py-2.5">
          ...
          <p className="text-xs font-medium text-gray-700">{formatDate(hotel.check_in)}</p>
          <p className="text-xs text-gray-400">to {formatDate(hotel.check_out)}</p>
          <p className="text-[10px] text-gray-400 mt-0.5">{nights()} night{nights() !== 1 ? "s" : ""}</p>
        </div>
        <div className="bg-gray-50 rounded-xl px-3 py-2.5">
          <div className="flex items-center gap-1.5 mb-1">
          ...
          <p className="text-lg font-bold text-gray-900">{hotel.guests}</p>
        </div>
        <div className="bg-gray-50 rounded-xl px-3 py-2.5">
          <span className="text-[10px] font-medium text-gray-400 uppercase tracking-wide">Price</span>
          <p className="text-lg font-bold text-gray-900 mt-1">${hotel.price?.toLocaleString()}</p>
        </div>
      </div>
    </div>
  );
